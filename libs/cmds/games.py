...
def coinflip(bot, user, side=None, *args):
	if side is None:
		bot.send_message("You need to guess which side the coin will land! (h,heads,t,tails)")
		...
		if side[0] == result[0]:
			db.execute("UPDATE users SET Coins = Coins + 30 WHERE UserID = ?",
				user["id"])
			bot.send_message(f"It landed on {result}! You won 30 coins!")
...
class Heist(object):
	...
	def add_user(self, bot, user, bet):
		if self.running:
			...
			bot.send_message("You're all suited and ready to go! Stand by for showtime... use !join to join")
